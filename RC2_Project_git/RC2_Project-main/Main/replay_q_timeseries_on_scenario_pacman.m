function replay_q_timeseries_on_scenario_pacman(type)
    ...
    % Salviamo le coordinate attive in vettori
    active_px = xy(idx,1);
    active_py = xy(idx,2);
    
    % Salviamo l'handle del plot dei punti
    ...
    % Raggio d'azione per "mangiare" i punti (basato sulla scala della mappa)
    eat_radius = min(Hh, Wh) * 0.03; % 3% della mappa
    eat_radius_sq = eat_radius^2;    % Eleviamo al quadrato per ottimizzare i calcoli
    ...
    for k = k0:timesteps:N
        ...
        if ~isempty(active_px)
            % Calcola la distanza al quadrato tra l'uniciclo e i punti rimasti
            dist_sq = (active_px - x(k)).^2 + (active_py - y(k)).^2;
            
            % Tieni solo i punti che sono più lontani del raggio di cattura
            keep_idx = dist_sq > eat_radius_sq;
            
            % Se l'array keep_idx contiene degli "0" (falso), significa che abbiamo mangiato qualcosa
            if any(~keep_idx)
                active_px = active_px(keep_idx);
                active_py = active_py(keep_idx);
                % Aggiorniamo i dati grafici togliendo i punti mangiati
