...
%% Trajectory Tracking Phase (L vs NL) ( t <= shift_time )
...
%% Regulation Phase (Cartesian vs Posture) ( t > shift_time )
...
plot(res.NL.t(idx_Cart_reg), dist_Cart, '-', 'Color', c_C, 'LineWidth', 1.5, 'DisplayName', 'Cartesian error');
plot(res.NL2.t(idx_Post_reg), dist_Post, '-', 'Color', c_P, 'LineWidth', 1.5, 'DisplayName', 'Posture error');
title('Distance Error'); xlabel('Time [s]'); ylabel('e_p [cm]'); xlim([shift_time, t_end_reg]);
lgd = legend('Location', 'best');
lgd.ItemHitFcn = @(~, evt) set(evt.Peer, 'Visible', ~strcmp(evt.Peer.Visible, 'on'));

% --- COLONNA SINISTRA BASSA: Linear velocity v ---
subplot(2, 3, 4); hold on; grid on;
plot(res.NL.t(idx_Cart_reg), res.NL.v(idx_Cart_reg), '-', 'Color', c_C, 'LineWidth', 1.5, 'DisplayName', 'Cartesian v');
plot(res.NL2.t(idx_Post_reg), res.NL2.v(idx_Post_reg), '-', 'Color', c_P, 'LineWidth', 1.5, 'DisplayName', 'Posture v');
title('Linear Velocity v'); xlabel('Time [s]'); ylabel('v [cm/s]'); xlim([shift_time, t_end_reg]);
lgd = legend('Location', 'best');
lgd.ItemHitFcn = @(~, evt) set(evt.Peer, 'Visible', ~strcmp(evt.Peer.Visible, 'on'));
...
plot(res.NL.t(idx_Cart_reg), res.NL.q(idx_Cart_reg, 3), '-', 'Color', c_C, 'LineWidth', 1.5, 'DisplayName', 'Cartesian \theta');
plot(res.NL2.t(idx_Post_reg), res.NL2.q(idx_Post_reg, 3), '-', 'Color', c_P, 'LineWidth', 1.5, 'DisplayName', 'Posture \theta');
title('Orientation Angle \theta'); xlabel('Time [s]'); ylabel('\theta [rad]'); xlim([shift_time, t_end_reg]);
lgd = legend('Location', 'best');
lgd.ItemHitFcn = @(~, evt) set(evt.Peer, 'Visible', ~strcmp(evt.Peer.Visible, 'on'));

% --- COLONNA DESTRA BASSA: Angular velocity w ---
subplot(2, 3, 6); hold on; grid on;
plot(res.NL.t(idx_Cart_reg), res.NL.w(idx_Cart_reg), '-', 'Color', c_C, 'LineWidth', 1.5, 'DisplayName', 'Cartesian \omega');
plot(res.NL2.t(idx_Post_reg), res.NL2.w(idx_Post_reg), '-', 'Color', c_P, 'LineWidth', 1.5, 'DisplayName', 'Posture \omega');
title('Angular Velocity \omega'); xlabel('Time [s]'); ylabel('\omega [rad/s]'); xlim([shift_time, t_end_reg]);
lgd = legend('Location', 'best');
lgd.ItemHitFcn = @(~, evt) set(evt.Peer, 'Visible', ~strcmp(evt.Peer.Visible, 'on'));